...
In Figure :numref:`schedule`, the directed acyclic graph on the left
...
:numref:`single_op_exec` shows the trace of operator scheduling. Once
...
specific device for execution.

.. _single_op_exec:

.. figure:: ../img/ch03/single_op_exec.png
   :width: 700px
...
-  **Serial**: executes its tasks one at a time, in the order that they
   ...
   fewer resources.
-  **Parallel**: executes its tasks concurrently for higher
...
must be guaranteed. Figure :numref:`order` shows a computational
...
each operator receives the necessary input.

.. _order:

.. figure:: ../img/ch03/order.png
   :width: 800px

   Serial operator scheduling



A computational graph may also contain operators independent of each
other, for example, op1 and op2 shown in Figure :numref:`para`. We can
...
.. _comparsion_execution:

.. figure:: ../img/ch03/para.png
   :width: 800px
...
disadvantages, as summarized in Table numref:\ ``comparsion_execution``.

:Comparison between serial execution and parallel execution \| Execution
Method \| Serial execution \| Parallel execution \| \| :—————–:\|
:————————————————–: \|:————————————————–: \| \| Execution Order \|
Static \| Dynamic \| \| Execution Threads \| Single thread \| Multiple
threads \| \| Resource Consumption \| Low \| High \|
...
-  **Synchronous**: Tasks are executed in order, one after the other.
   Tasks have to wait for and coordinate between each other.
-  **Asynchronous**: When a task is complete, the same task in the next
   iteration can be executed immediately.

If the synchronous mechanism is adopted to train the computational graph
shown in Figure :numref:`synchronization`, in each iteration, a batch
...
In the asynchronous setting shown in Figure :numref:`asynchronous`,
...
computing, as shown in Figure :numref:`asyn_para`. On the one hand,
...
.. figure:: ../img/ch03/para-async.png
   :width: 800px

   Asynchronous mechanism combined with parallel computing
