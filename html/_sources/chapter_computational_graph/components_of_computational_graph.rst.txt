...
dependencies between operators. :numref:`simpledag`\ shows a
computational graph example of
:math:`\boldsymbol{Z}=ReLU(\boldsymbol{X}\times\boldsymbol{Y})`.

.. _simpledag:

.. figure:: ../img/ch03/simple-graph.png
   :width: 300px
...
image is a rank-3 tensor with three axes. See :numref:`tensor`.

.. _tensor:

.. figure:: ../img/ch03/tensor.png
   :width: 800px
...
gradient transfer status. :numref:`tensor_attr`\ describes the main
...
.. table:: Tensor attributes
   ...
   +===============================================+=======================+
   | shape                                         | Length of each        |
   |                                               | dimension, for        |
   |                                               | example, [3,3,3].     |
   +-----------------------------------------------+-----------------------+
   | dim                                           | Number of axes (or    |
   |                                               | dimensions). The      |
   |                                               | value is 0 for a      |
   |                                               | scalar and 1 for a    |
   |                                               | vector.               |
...
:numref:`tensorclass`, a tensor is ragged if it has variable numbers
...
.. figure:: ../img/ch03/tensor-class.png
   :width: 800px
...
-  **Tensor operators** involve tensor structure and mathematical
...
-  **Neural network operators**, the foundation of neural network
...
-  **Data flow operators**\ cover data preprocessing and loading. Data
...
-  **Control flow operators**, usually found in flexible and complex
...
As shown in Figure :numref:`dependence`, if the Matmul1 operator is
...
.. figure:: ../img/ch03/dependence.png
   :width: 400px
...
-  **Direct dependency**: For example, the ReLU1 node is directly
   dependent on the Matmul1 node. That is, ReLU1 can run properly only
   when it receives a direct output from Matmul1.

-  **Indirect dependency**: For example, the Add node indirectly depends
...
-  **Mutual independence**: For example, the graph shows no input/output
   dependency between Matmul1 and Matmul2, meaning that the two nodes
   are independent of each other.

In the computational graph shown in Figure :numref:`recurrent`, the
...
represent loop iterations. Figure :numref:`unroll` shows a
...
-  **Frontend control flows**: Python control flow statements are used
...
-  **Framework control primitives**: Machine learning frameworks come
...
Code shows an example of simple conditional control, where ``matmul``
...
Figure :numref:`if`\ depicts the forward and backward computational
graphs of above code. When running a model containing ``if`` conditions,
...
the forward computational graph, tensor :math:`\boldsymbol{C}` does not
participate in computation due to conditional control. Similarly, in the
backward computational graph, tensor :math:`\boldsymbol{C}` is skipped
in gradient computation.

.. _if:

.. figure:: ../img/ch03/if.png
   :width: 600px
...
control loop example. In its unrolled loop, :math:`\boldsymbol{X_i}` and
:math:`\boldsymbol{W_i}` are the lists of intermediate result tensors to
be maintained.
...
Figure :numref:`while`. The gradient of the control loop is also a
...
:math:`\boldsymbol{X}` is fed into the neural network, the data is
...
:math:`\boldsymbol{Y}` is output after multilayer computation. In DNN
...
:eq:`ch04-1` of composite functions with respect to scalars as
follows: Assuming that :math:`\boldsymbol{X}` is an *m*-dimensional
tensor,\ :math:`\boldsymbol{Y}` is an *n*-dimensional tensor,
:math:`\boldsymbol{z}` is a 1D vector,
:math:`\boldsymbol{Y}=g(\boldsymbol{X})`, and
:math:`\boldsymbol{z}=f(\boldsymbol{Y})`, the partial derivative of
:math:`\boldsymbol{z}` with respect to each element of
:math:`\boldsymbol{X}` is

.. math:: \frac{\partial z}{\partial x_i}=\sum_j\frac{\partial z}{\partial y_j}\frac{\partial y_j}{\partial x_i}
   :label: ch04-2

The equivalent form of Equation :eq:`ch04-2` is

.. math:: \nabla_{\boldsymbol{X}}\boldsymbol{z} = (\frac{\partial \boldsymbol{Y}}{\partial \boldsymbol{X}})^{\top}\nabla_{\boldsymbol{Y}}\boldsymbol{z}
   :label: ch04-3

where, :math:`\nabla_{\boldsymbol{X}}\boldsymbol{z}` represents the
gradient matrix of :math:`\boldsymbol{z}` with respect to
:math:`\boldsymbol{X}`.

Figure :numref:`chain` shows the application of the chain rule in
...
obtain the predicted value :math:`\boldsymbol{Y}`, and then performs
...
weight parameters to be updated include :math:`\boldsymbol{W}` and
:math:`\boldsymbol{W_1}`.

.. _chain:

.. figure:: ../img/ch03/chain.png
   :width: 600px
...
transfer the gradient information to :math:`\boldsymbol{W}` and
:math:`\boldsymbol{W_1}` using the chain rule method? And why do we need
to calculate the gradients of non-parameter data :math:`\boldsymbol{X}`
and :math:`\boldsymbol{X_1}`? To answer these questions, let’s analyze
...
:math:`\boldsymbol{X_1}=\boldsymbol{XW}`; (2)
:math:`\boldsymbol{Y}=\boldsymbol{X_1W_1}`; and (3)
Loss=\ :math:`\frac{1}{2}`\ (:math:`\boldsymbol{Y}`-Label):math:`^2`.
...
backpropagation is performed through Equations :eq:`ch04-4` and
:eq:`ch04-5` to calculate the gradients of the loss function with
respect to parameters :math:`\boldsymbol{W}` and
:math:`\boldsymbol{W_1}`:

.. math:: \frac{\partial {\rm Loss}}{\partial \boldsymbol{W_1}}=\frac{\partial \boldsymbol{Y}}{\partial \boldsymbol{W_1}}\frac{\partial {\rm Loss}}{\partial \boldsymbol{Y}}
   :label: ch04-4

.. math:: \frac{\partial {\rm Loss}}{\partial \boldsymbol{W}}=\frac{\partial \boldsymbol{X_1}}{\partial \boldsymbol{W}}\frac{\partial {\rm Loss}}{\partial \boldsymbol{Y}}\frac{\partial \boldsymbol{Y}}{\partial \boldsymbol{X_1}}
   :label: ch04-5

Both Equations :eq:`ch04-4` and :eq:`ch04-5` solve
:math:`\frac{\partial {\rm Loss}}{\partial \boldsymbol{Y}}`, which
corresponds to grad :math:`\boldsymbol{Y}` in Figure :numref:`chain`.
:math:`\frac{\partial {\rm Loss}}{\partial \boldsymbol{Y}}\frac{\partial \boldsymbol{Y}}{\partial \boldsymbol{X_1}}`
in Equation :eq:`ch04-5` corresponds to grad grad
:math:`\boldsymbol{X_1}` in Figure :numref:`chain`. To calculate the
gradient of model parameter :math:`\boldsymbol{W}`, the gradient of
intermediate result :math:`\boldsymbol{X_1}` is calculated. This also
...
Because :math:`\boldsymbol{X_1}=\boldsymbol{XW}`,
:math:`\boldsymbol{Y}=\boldsymbol{X_1W_1}`, and
Loss=\ :math:`\frac{1}{2}`\ (:math:`\boldsymbol{Y}`-Label):math:`^2`,
Equations :eq:`ch04-4` and :eq:`ch04-5` are expanded to
:eq:`ch04-6` and :eq:`ch04-7` according to Equations
:eq:`ch04-3`, respectively. Then, we can analyze how variables
...
.. math:: \frac{\partial {\rm Loss}}{\partial \boldsymbol{W_1}}=\frac{\partial \boldsymbol{Y}}{\partial \boldsymbol{W_1}}\frac{\partial {\rm Loss}}{\partial \boldsymbol{Y}}=\boldsymbol{X_1}^\top(\boldsymbol{Y}-{\rm Label})
   :label: ch04-6

.. math:: \frac{\partial {\rm Loss}}{\partial \boldsymbol{W}}=\frac{\partial \boldsymbol{X_1}}{\partial \boldsymbol{W}}\frac{\partial {\rm Loss}}{\partial \boldsymbol{Y}}\frac{\partial \boldsymbol{Y}}{\partial \boldsymbol{X_1}}=\boldsymbol{X}^\top(\boldsymbol{Y}-{\rm Label})\boldsymbol{W_1}^\top
   :label: ch04-7

Equation :eq:`ch04-6` uses intermediate result
:math:`\boldsymbol{X_1}` in the forward computational graph when
calculating the gradient of :math:`\boldsymbol{W_1}`. In equation
:eq:`ch04-7`, both input :math:`\boldsymbol{X}` and parameter
:math:`\boldsymbol{W_1}` are used for calculating the gradient of
parameter :math:`\boldsymbol{W}`. This answers the first question. The
...
Based on Figure :numref:`chain` and Equations :eq:`ch04-4`,
:eq:`ch04-5`, :eq:`ch04-6` and :eq:`ch04-7`, when the
